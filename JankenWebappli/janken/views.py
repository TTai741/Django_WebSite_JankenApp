    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user and not user.is_superuser:
                ...
                # 管理者の場合はログインページにリダイレクト
                return redirect(to='/jankenApp/login/')
...
def ranking_view(request):
    # 管理者ではないユーザーのみを取得
    users = User.objects.filter(is_staff=False, is_superuser=False).order_by('-win_rate')
